  ...
  return (
    <div className="bg-card border border-border/20 rounded-xl px-3 py-2 shadow-card text-xs">
      ...
      <p className="text-muted-foreground mt-0.5">
        <span className="font-bold text-foreground">{d.value}</span> clients
      </p>
    </div>
  );
};
    ...
    <div className="bg-card/30 backdrop-blur-xl border border-border/10 rounded-3xl p-6 shadow-elegant space-y-4">
      <div className="flex items-center justify-between">
        <div className="space-y-0.5">
          ...
          <p className="text-[11px] text-muted-foreground">{total} total clients</p>
        </div>
        <Link
          href="/admin/clients"
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-2">
            {data.map((d) => {
              ...
              return (
                <Link
                  key={d.status}
                  href={`/admin/clients?status=${d.status}`}
